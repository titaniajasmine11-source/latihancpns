      ...
      <section className="mx-auto flex w-full max-w-4xl flex-col gap-5">
        <header className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
            Tampilan read-only untuk skor, batas latihan, dan limit generator. Edit nilai melalui Supabase SQL Editor sampai form pengaturan tersedia.
          </p>
        </header>
        ...
        {(settings as AppSetting[] | null)?.length ? (
          <div className="grid gap-4">
            {(settings as AppSetting[]).map((setting) => (
              <article className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm" key={setting.key}>
                ...
                <pre className="mt-4 overflow-x-auto rounded-2xl bg-slate-950 p-4 text-sm leading-6 text-emerald-100">
